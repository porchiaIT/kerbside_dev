 ...
 */
function conch_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'conch_form_submit';
}


function conch_form_submit($form, &$form_state) {
  // Import database conch file.
  $conch_file = dirname(__FILE__) . '/conch.sql';
  $success = import_conch($conch_file);
...
/**
 * Imports a database conch file.
 *
 * @see demo_reset().
 */
function import_conch($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open conch file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
